 ...
 *
 * There are three main additions to the starter kit bot code: the mecanum drive, a linear slide for reaching
 * into the submersible and a limelight for detecting samples and aligning the robot based on sample color.
 *
 * The drive system is all 5203-2402-0019 (312 RPM Yellow Jacket Motors), and it is based on a Strafer mecanum chassis
...
@TeleOp(name="Limelight_Basket_TeleOp", group="!FTC")
//@Disabled
public class OrfLimelightTeleOp extends LinearOpMode {
    ...
    private Limelight3A limelight;  //limelight object

    // PID coefficients for rotation alignment with the limelight camera
    ...
    double oldTime = 0;
    // limelight variables
    double limelightIntegral = 0, limelightLastError = 0;
    //odometry sensor
    ...
    public void runOpMode() {
       ...
       // initializeIMU();
        ...
        /* Having finished the initialization process, the program
        waits for the game driver to press the play button in the driver station console*/
        waitForStart();

        /* Runs continuously until the driver presses stop in the driver station */
        while (opModeIsActive())
        {
            // driving in field-centric strafing mode
            straferMovement();
...
//            setMinArmPos();
            ...
            if(gamepad2.a){ // ps4: X
                ...
                back to be folded inside the robot. */
                armCollapsed();
                wristHorizontal();
                intakeOpen();
            }
            else if (gamepad2.dpad_right){
                /* This is a limelight-guided sample alignment collection process*/
               limelightSampleAlign();
            }
            ...
            position. This works a lot like our arm. When we click a button, it moves to a position and then stops.
            But the drivers wanted more "open loop" controls. So we want the lift to keep extending for
            as long as we hold the bumpers, and when we let go of the bumper, stop where it is.
...
public void initializeLimelight() {
        ...
        limelight.start();
        limelight.pipelineSwitch(0);  // configure Yellow color blob pipeline via Web UI
        }
...
//        telemetry.addData("arm position", armPosition);
    ...
    public void armClearBarrier() {
        wrist.setPosition(0);
            ...
            they were doing before we clicked the left bumper. */
        armPosition = armDegreesToTicks(20);
    }
    //These are the functions for limelight handling
    /* This is a limelight-guided sample alignment collection process*/
    public void limelightSampleAlign() {
        // Reset timer and PID state


            LLResult result = limelight.getLatestResult();
            if (result != null && result.isValid()) {
                double tx = result.getTx();  // horizontal angle to yellow sample
                double ty = result.getTy();  // vertical angle to yellow sample

                // PID loop for steer adjustment
                double error = tx-12; // 12 is the offset of intake center from the limelight camera center
                limelightIntegral += error * getLoopDuration();  // simple limelightIntegral
                double derivative = (error - limelightLastError) / getLoopDuration();
                double turnPower = error*kP_TURN + limelightIntegral *kI_TURN + derivative*kD_TURN;
                limelightLastError = error;

                // Strafe power based on lateral distance (ty), if needed:
                double strafePower = ty * 0.00;  // tune constant

                  // Drive robot to align and strafe
                drive(turnPower, strafePower);

                // If within target thresholds, activate intake
                if (Math.abs((tx-12)) < 2.0 && (Math.abs(ty+15)) < 1.5) { // substract offsets tx -12 ty +15
                   armClearBarrier();
                   intakeOpen();
                   sleep(500);
                   intakeCollect();
                ...
                telemetry.addData("last error", limelightLastError);
                telemetry.addData("strafePwr", strafePower);
                telemetry.addData("turnPwr", turnPower);
//                telemetry.update();

            } else {
                // if no valid target, stop moving
                drive(0, 0);
                 /* This is the starting configuration of the robot. This turns off and opens fully the intake,and moves the arm
        ...
        // Helper: drive robot with rotation & strafe (custom to your chassis)
    private void drive(double turn, double strafe) {
        // e.g. mecanum: set motor powers accordingly
        leftFrontDrive.setPower(strafe + turn);
        rightBackDrive.setPower(strafe + turn);
        rightFrontDrive.setPower(strafe - turn);
        leftBackDrive.setPower(strafe - turn);

    }
     // loop duration in seconds, assume ~30 Hz
    private double getLoopDuration() { return 1.0/30.0; }
